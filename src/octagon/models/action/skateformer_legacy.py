"""Legacy SkateFormer model (simplified architecture).

DEPRECATED: This is the legacy simplified SkateFormer implementation.
Use the new `skateformer.py` for the official KAIST architecture that
supports pretrained weight loading.

This module is kept for backward compatibility. New code should use:
    from octagon.models.action import SkateFormer  # Official architecture

This simplified version:
- Uses flat block structure instead of stages
- Uses single global attention instead of 4-way partition attention
- Does NOT support loading official pretrained weights
"""
import math
import warnings
...
class SkateFormerLegacy(nn.Module):
    ...
    def __init__(
        ...
        warnings.warn(
            "SkateFormerLegacy is deprecated. Use SkateFormer from "
            "octagon.models.action.skateformer for official architecture "
            "with pretrained weight support.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def from_pretrained(
        ...
        NOTE: This legacy implementation cannot load official KAIST pretrained
        weights due to architecture mismatch. Use SkateFormer from skateformer.py
        ...
        Returns:
            SkateFormerLegacy model with pre-trained weights loaded.

        Example:
            >>> model = SkateFormerLegacy.from_pretrained(
...
class SkateFormerWrapperLegacy(BaseActionRecognizer):
